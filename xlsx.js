...
function write_ws_xml_datavalidation_formula(validations) {
	var o = '<dataValidations>';
	for (var i = 0; i < validations.length; i++) {
		var validation = validations[i];
		o += '<dataValidation type="list" allowBlank="0" ';
		if (validation.errorTitle && validation.errorMessage) {
			o += 'showErrorMessage="1" errorTitle="' + validation.errorTitle + '" error="' + validation.errorMessage + '" ';
		}
		...
		o += '<formula1><xm:f>' + validation.sheetRef + '</xm:f></formula1>';
		o += '</dataValidation>';
	}
	o += '</dataValidations>';
	return o;
...
function make_xlsx_lib(XLSX) {
	...
	function write_ws_xml(idx, opts, wb, rels) {
		...
		if (ws['!dataValidationFormula']) o[o.length] = write_ws_xml_datavalidation_formula(ws['!dataValidationFormula']);
		/* phoneticPr */
